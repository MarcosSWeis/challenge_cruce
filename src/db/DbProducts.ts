...
export class dbProducts {
  ...
  getAllProductsPaginates(
    limit: number,
    page: number,
    arrToPaginate: Array<Product>
  ): Paginate {
    const offset = limit * page;
    const previousOffset = limit * (page - 1);
    if (page == 1) {
      return new Paginate(
        arrToPaginate.length,
        limit,
        page,
        arrToPaginate.slice(page - 1, limit)
      );
    } else {
      return new Paginate(
        arrToPaginate.length,
        limit,
        page,
        arrToPaginate.slice(previousOffset, offset)
      );
    }
  }
  ...
  getProductsByCategory(category: CategoryProduct): Array<Product> | undefined {
    ...
    if (category.toy) {
      return this.products.filter(
        (product) => product.category.toy === category.toy
      );
    }
    if (category.school) {
      return this.products.filter(
        (product) => product.category.school === category.school
      );
    }
  }
  //obtener el id mayor
  ...
  getFilterDiscount(discount: number): Array<Product> | undefined {
    return this.products.filter(
      (product) => product.price.discount <= discount
    );
  }
